#!/usr/bin/python
# -*- coding: utf-8 -*-
import csv, requests, json, telepot, sys, os, time, datetime, subprocess, psutil, RPi.GPIO as GPIO
...
logfile = "botlog.txt"
userfile = "users.csv"
grantfehler = "tztztz ... Du darfst das nicht!"
mmdvmaufruf = "sudo systemctl start mmdvmhost.service"
dmrgwaufruf = "sudo systemctl start dmrgateway.service"
svxaufruf = "sudo systemctl start svxlink"
svxstop = "sudo systemctl stop svxlink"
link341 = "sudo sh /home/pi/remote/link341.sh"
link951 = "sudo sh /home/pi/remote/link951.sh"
...
def lastheared(suchstring):
    ...
    file.close()
    if not heared:
	return "Bis jetzt hat noch keiner Gefunkt :-( ..."
    else:
...
## Svxlink Log
        ...
        lhthr = subprocess.check_output('grep "THR_Logic: Talker" /var/log/svxlink.log | tail -1 | cut -d: -f6', shell=True)
        bot.sendMessage(chat_id,"Zuletzt gehört im ThüringenLink:" + lhthr)
##
    elif msg['text'] in ["/lastata"]:
        lhata = subprocess.check_output('grep "ATA_Logic: Talker" /var/log/svxlink.log | tail -1 | cut -d: -f6', shell=True)
        bot.sendMessage(chat_id,"Zuletzt gehört im Verbund Berlin:" + lhata)

    elif msg['text'] in ["/lastecho"]:
        lhecho = subprocess.check_output('grep "EchoLink QSO state changed to CONNECTED" /var/log/svxlink.log | tail -1 | cut -d: -f4', shell=True)
